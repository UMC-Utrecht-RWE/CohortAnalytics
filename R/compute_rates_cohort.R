...
compute_rates_cohort <- function(
  ...
  # sum outcomes
  n_out_exp <- aesifup_input[
    aesifup_input$group == "EXPOSED",
    sum(.SD[[eventCol]])
  ]
  n_out_con <- aesifup_input[
    aesifup_input$group == "CONTROL",
    sum(.SD[[eventCol]])
  ]
  ...
  model_formula_no_group <- stats::as.formula(paste0(
    eventCol,
    " ~ offset(",
    pyr_offset,
    ")"
  ...
  if (weighted_IR == TRUE) {
    ...
    ir_list_exp <- est_inc_prev_model(
      model_adj_exposed,
      group = "EXPOSED",
      scale_IR = scale_IR
    )
  } else {
    ir_list_exp <- est_inc_prev(
      n_pat_exp,
      n_out_exp,
      py_exp,
      scale_IR = scale_IR,
  ...
  if (model_based_control) {
    if (weighted_IR == TRUE) {
      ...
      ir_list_con <- est_inc_prev_model(
        model_adj_control,
        group = "CONTROL",
        scale_IR = scale_IR
      )
    } else {
      model_crude_control <- fitmod_gee(
        model_formula_no_group,
        model_type = "crude",
        aesi_name = target_aesi,
        aesifup_input = aesifup_input[aesifup_input$group == "CONTROL", ]
  ...
  if (risk_type == "survival" & CumulativeInc) {
    ...
    cuminc_ests_crude <- create_risk_table(
      aesifup_input,
    ...
    if (!is.null(iptw)) {
      cuminc_ests_adj <- create_risk_table(
        aesifup_input,
        timepoints = max_fuptime,
        target_aesi = target_aesi,
        use_weights = TRUE,
  ...
  colnames(cuminc_ests_crude)[-1] <- paste0(
    colnames(cuminc_ests_crude)[-1],
    "_crude"
  )
  ...
  if (!comparison_measures) {
    ...
    if (nrow(aesifup) == 0) {
      ...
      if (risk_type == "survival") {
        ...
        rd_est_crude <- cuminc_ests_crude$cuminc_est_exp -
          cuminc_ests_crude$cuminc_est_con
        rd_est_adj <- cuminc_ests_adj$cuminc_est_exp -
          cuminc_ests_adj$cuminc_est_con

        if (is.na(rd_est_crude)) {
          rd_est_crude <- -88
        }
        if (is.na(rd_est_adj)) {
          rd_est_adj <- -88
        ...
        if (!is.null(minimum_count_for_comparative)) {
          if (
            n_out_exp < minimum_count_for_comparative |
              n_out_con < minimum_count_for_comparative
          ) {
            rd_list <- data.frame(
              rd_est_crude = -77,
              rd_lb_crude = -77,
        ...
        colnames(risk_ratio_diff) <- gsub(
          "irr",
          "rr",
          colnames(risk_ratio_diff)
        )
        colnames(risk_ratio_diff) <- gsub(
          "ird",
          "rd",
